    ...
    public class ObjectStateDropdown : DSDropDownBase
    {
        ...
        public override IEnumerable<AssociativeNode> BuildOutputAst(List<AssociativeNode> _)
        {
            ...
            var callNode = AstFactory.BuildFunctionCall(
                new Func<string, ObjectState>(ParseObjectState),
        ...
        private static ObjectState ParseObjectState(string stateString)
        {
            if (Enum.TryParse<ObjectState>(stateString, out var result))
            {
                return result;
            }
            return ObjectState.Alive; // Возвращаем Alive как значение по умолчанию
        }
    }
} 